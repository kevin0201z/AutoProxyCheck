...
- `Install_ScheduledTasks.ps1`：一键注册或更新计划任务的安装脚本
...
3. 使用管理员权限运行一键安装脚本，或按后文手动创建计划任务。
...
推荐使用项目根目录中的 `Install_ScheduledTasks.ps1` 自动注册计划任务。

普通 PowerShell 或管理员 PowerShell 都可以运行：

```powershell
powershell.exe -ExecutionPolicy Bypass -File ".\Install_ScheduledTasks.ps1"
```

如果当前窗口不是管理员权限，脚本会自动触发 UAC 提权并重新运行自身。

安装脚本会注册或更新以下两个计划任务：

- `AutoProxyCheck_Background`：系统启动或用户登录时以 `SYSTEM` 身份运行后台检测脚本。
- `AutoProxyCheck_Tray`：当前用户登录时运行托盘脚本。

这两个任务会放在任务计划程序库的 `AutoProxyCheck` 文件夹下。
如果之前旧版本曾把同名任务安装在任务计划程序库根目录，安装脚本会自动移除根目录旧任务。
托盘任务启动时会检查后台任务状态；如果后台任务没有运行，会尝试启动 `AutoProxyCheck_Background`。

默认安装完成后会立即启动这两个任务，方便马上验证。如果只想注册任务、不立即启动，可以执行：

```powershell
powershell.exe -ExecutionPolicy Bypass -File ".\Install_ScheduledTasks.ps1" -NoStart
```

脚本会使用当前项目目录作为部署目录。如果之后移动项目目录，需要重新运行安装脚本，让计划任务中的脚本路径同步更新。

可用以下命令检查计划任务是否已注册：

```powershell
Get-ScheduledTask -TaskPath "\AutoProxyCheck\" -TaskName AutoProxyCheck_Background, AutoProxyCheck_Tray
```
...
托盘脚本启动时会检查后台任务状态；如果后台任务没有运行，会尝试启动 `AutoProxyCheck_Background`。
...
如果在任务计划程序中手动运行托盘任务但看不到托盘图标，请先确认：

- 托盘任务必须设置为“仅当用户登录时运行”，不能设置为“不管用户是否登录都要运行”。
- 任务动作应指向当前部署目录中的 `Start_ProxyTray_Hidden.vbs`。
- `C:\ProgramData\AutoProxyCheck\AutoProxyCheck.log` 中应出现 `[TrayLauncher]` 日志；如果没有，说明计划任务动作没有执行到 VBS 启动器。
- 如果有 `[TrayLauncher]` 但没有 `[Tray]` 日志，说明 VBS 已启动但 PowerShell 托盘脚本启动失败，可手动运行 `ProxyTray_UI.ps1` 查看错误。
